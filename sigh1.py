...
for sampleNumber in range(0,(IN_TIME*CHUNK_TIME)):
	out_lut[sampleNumber] = int(interp(sampleNumber, [0,(IN_TIME*CHUNK_TIME)], [AMPLITUDE/2,AMPLITUDE]))

for sampleNumber in range((IN_TIME*CHUNK_TIME), ((IN_TIME*CHUNK_TIME) + (OUT_TIME*CHUNK_TIME))):
	out_lut[sampleNumber] = int(interp(sampleNumber, [(IN_TIME*CHUNK_TIME),((OUT_TIME*CHUNK_TIME)+(IN_TIME*CHUNK_TIME))], [AMPLITUDE, 0]))

for sampleNumber in range( ((IN_TIME*CHUNK_TIME) + (OUT_TIME*CHUNK_TIME)),(2*CHUNK_TIME*(IN_TIME+OUT_TIME))):
	out_lut[sampleNumber] = int(interp(sampleNumber, [((OUT_TIME*CHUNK_TIME)+(IN_TIME*CHUNK_TIME)),2*CHUNK_TIME*(IN_TIME+OUT_TIME)], [0, AMPLITUDE/2]))
