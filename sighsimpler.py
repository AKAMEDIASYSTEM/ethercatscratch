...
for sampleNumber in range( BREATHE_OUT_START,BREATHE_OUT_END):
	out_lut[sampleNumber] = int(interp(sampleNumber, [BREATHE_OUT_START,BREATHE_OUT_END], [MAX_BREATHE_RATE, AMPLITUDE/5]))
# reset to mid-amplitude
for sampleNumber in range(BREATHE_OUT_END, BREATHE_OUT_END+ RESET_TIME):
	out_lut[sampleNumber] = int(interp(sampleNumber, [BREATHE_OUT_END, BREATHE_OUT_END+ RESET_TIME], [AMPLITUDE/5, 0]))
